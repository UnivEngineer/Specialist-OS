...
образы записываются в `ROM`.

Каноническая коллекция игр находится в `assets\games`, а в `FlashDrive`
создаются образы для прошивания flash-диска. Состав вариантов `Games64k` и
`Games2M` задаётся манифестами; проверены не все игры.
