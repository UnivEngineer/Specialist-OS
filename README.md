...
SjASMPlus через PowerShell-сценарии из `scripts\build`.
...
программатор ПЗУ. Задача VS Code `image: system ROM (hardware)` собирает
аппаратный образ объёмом 128, 256 или 512 КБ. Входные файлы пока находятся в
`ROM\makeRom`, а готовые образы — в `ROM`.

В папке FlashDrive - набор образов для прошивания Флеш-диска, и скрипт для сборки образов из файлов. Приложена коллекция игр для Специалиста почти на 2 Мб (проверены не все!).
