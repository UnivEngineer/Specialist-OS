...
- Экспериментальный драйвер флеш-диска с параллельной записью во все 4 чипа AT29C040A. Теоретически - усеорение в 4 раза; на практике - скорость упирается в медленный обмен через ВВ55, реальное ускорение - в 2 раза.

FLASHSYS.COM (NEW)

- Драйвер перезаписываемого системного ROM-диска со страничным маппингом в адреесное пространство CPU. Поддерживаются микросхемы объёмом 128, 256 и 512 КБ.
...
- Для эмулятора H: устанавливается командой `A:ROM.COM` и использует образ 64-КБ ROM-диска. На физической машине с AT29C040A следует устанавливать `A:FLASH.COM` или `A:FLSHFAST.COM`.
